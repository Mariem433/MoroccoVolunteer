 ...
 */
@WebServlet(name = "RequestsPage", urlPatterns = {"/requestspage"})
public class RequestsPage extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<link rel=\"stylesheet\" href=\""+request.getContextPath()+"/assets/css/styles.min.css\">");
                    ...
                    if (conn != null) {
                        if(role.equals("Organization") && role!=null){
                              ...
                              ResultSet rst1 = stmt1.executeQuery("SELECT "
                                      ...
                                      + " INNER JOIN POSITION AS P ON P.eventId= E.eventId NATURAL JOIN REQUEST As R"
                                      + " WHERE requestStatus = 'pending' AND O.organizationId = "+Id);
      ...
      "                     </div>");
                                  out.println("</div>\n");
                                      out.print("<form action='./cancelrequest'>");
                            ...
                            Statement stmt1 = conn.createStatement();
                            ResultSet rst1 = stmt1.executeQuery("SELECT "
                                    + "organizationName, role, E.name, field, positionId, volunteerId "
                                    + " From Event AS E INNER JOIN Organization AS O ON E.organizationId = O.organizationId  "
                                    + " INNER JOIN POSITION AS P ON P.eventId= E.eventId NATURAL JOIN REQUEST As R"
    ...
    "                     </div>");
                                out.println("</div>\n");
                                    ...
                                    out.println(" <button class=\"btn btn-primary overlay\" type=\"submit\"><i class=\"fas fa-plus\"></i>&nbsp;Decline</button>\n");
                                    out.println(" <button class=\"btn btn-primary overlay\" type=\"submit\" formaction= './acceptrequest'><i class=\"fas fa-plus\"></i>&nbsp;Accept</button>\n" +
                                    "</form>\n");
                                out.println("</div>");  
                                out.println("</div>");
                            }
